...
=head1 History
  ...
  Mender: Wujiao Li hnnd059@gmail.com
  Version: 5.0,  Date: 2007-12-25
  Version: 5.1,  Date: 2015-04-13
=head1 Usage
  ...
  --scaffold	      change to scaffold mode, get things belonged to same scaffold
...
my ($Baitformat,$FishFormat,$BaitColumn,$FishColumn,$Except,$PatternMode,$GeneMode,$scaffold);
my ($Verbose,$Help);
GetOptions(
	"bformat:s"=>\$Baitformat,
	"fformat:s"=>\$FishFormat,
	"bsep:s"=>\$BaitSeperator, # add by 59
	"fsep:s"=>\$FishSeperator,  # add by 59
...
sub output_gff{
	...
	while (<IN>) {
		...
		$id = $temp[0] if($scaffold); #add by pcj;
		#print "$id\t$scaffold\n";
		if (!$Except) {
